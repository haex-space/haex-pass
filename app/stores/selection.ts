...
export const useSelectionStore = defineStore('selection', () => {
  ...
  const currentGroupItems = computed<IPasswordMenuItem[]>(() => {
    ...
    const filteredGroups = search
      ...
      : groups.filter((group) => group.parentId == currentGroupId && group.id !== 'trash')
  ...
  const selectAll = () => {
    currentGroupItems.value.forEach((item) => selectedItems.value.add(item.id))
    isSelectionMode.value = true
  }
  ...
  const isCut = (itemId: string) => {
    return clipboardMode.value === 'cut' && clipboardItems.value.some((item) => item.id === itemId)
  }
